pipeline {
    ...
    stages {
        ...
        stage('GCloud Login'){
            steps{
                withCredentials([file(credentialsId: 'owneraccess-terraform', variable: 'GC_KEY')]) {
        ...
        stage('Terraform GKE cluster Plan and Apply') {
            steps   {
                script {
                    ...
                    --var 'project_id=astute-veld-344810' \
                    --var 'region=us-central1'\
                    --var 'cluster_name=terraform-test' && terraform -chdir=gke-cluster apply --input=false --auto-approve tfplan '''
